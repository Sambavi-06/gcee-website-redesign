...
const startIdx = content.indexOf('const deptData = {');
const endIdx = content.indexOf('// Provide a fallback for other departments');
if (startIdx === -1 || endIdx === -1) throw new Error("Could not find boundaries");

let deptDataStr = content.substring(startIdx + 'const deptData = '.length, endIdx).trim();
const deptDataObjStr = deptDataStr.endsWith(';') ? deptDataStr.slice(0, -1) : deptDataStr;
let deptData = eval('(' + deptDataObjStr + ')');
...
const newDeptDataStr = JSON.stringify(deptData, null, 2);
...
let finalContent = content.substring(0, startIdx) + 
                   "const deptData = " + newDeptDataStr + ";\n" +
                   content.substring(endIdx, modalLogicStart) +
                   newModalLogic + 
